...
const INITIAL_CHARACTERS: Character[] = [
  { id: 'c1', name: 'Arthur', role: 'Cavaleira', xp: 0, color: 'text-amber-500', bgColor: 'bg-amber-500/20', borderColor: 'border-amber-500', glowColor: 'drop-shadow-[0_0_15px_rgba(245,158,11,0.8)]' },
  { id: 'c2', name: 'Merlin', role: 'Mago', xp: 0, color: 'text-blue-500', bgColor: 'bg-blue-500/20', borderColor: 'border-blue-500', glowColor: 'drop-shadow-[0_0_15px_rgba(59,130,246,0.8)]' },
  { id: 'c3', name: 'Robin', role: 'Arqueira', xp: 0, color: 'text-emerald-500', bgColor: 'bg-emerald-500/20', borderColor: 'border-emerald-500', glowColor: 'drop-shadow-[0_0_15px_rgba(16,185,129,0.8)]' },
  { id: 'c4', name: 'Loki', role: 'Ladino', xp: 0, color: 'text-red-500', bgColor: 'bg-red-500/20', borderColor: 'border-red-500', glowColor: 'drop-shadow-[0_0_15px_rgba(239,68,68,0.8)]' },
  { id: 'c5', name: 'Elora', role: 'Clériga', xp: 0, color: 'text-stone-200', bgColor: 'bg-stone-200/20', borderColor: 'border-stone-200', glowColor: 'drop-shadow-[0_0_15px_rgba(231,229,228,0.8)]' },
];
...
const getSolidIcon = (type: SolidType) => {
  switch(type) {
    case 'Cubo': return <Cubo3D size={120} className="drop-shadow-2xl" />;
    case 'Cilindro': return <Cilindro3D size={120} className="drop-shadow-2xl" />;
    case 'Paralelepípedo': return <Paralelepipedo3D size={120} className="drop-shadow-2xl" />;
    case 'Pirâmide': return <Piramide3D size={120} className="drop-shadow-2xl" />;
    case 'Esfera': return <Esfera3D size={120} className="drop-shadow-2xl" />;
    case 'Cone': return <Cone3D size={120} className="drop-shadow-2xl" />;
    case 'Tetraedro': return <Tetraedro3D size={120} className="drop-shadow-2xl" />;
    case 'Prisma Pentagonal': return <PrismaPentagonal3D size={120} className="drop-shadow-2xl" />;
    case 'Prisma Hexagonal': return <PrismaHexagonal3D size={120} className="drop-shadow-2xl" />;
    case 'Cilindro-Cone': return <CilindroCone3D size={140} className="drop-shadow-2xl" />;
  }
};
...
const CharacterAvatar = ({ role, className }: { role: string, className?: string }) => {
  const imageMap: Record<string, string> = {
    'Arqueira': '/arqueira.jpg',
    'Cavaleira': '/cavaleiro.jpg',
      ...
      if (newHp <= 0) {
        if (bossStage === 1) {
          ...
          setTimeout(() => {
            ...
            setCurrentProblem(generateProblem(['Cilindro-Cone'] as SolidType[]));
            setInputValue('');
          }, 1500);
        } else {
          // Boss 2 defeated!
        ...
        // Next problem
        const validTypes = bossStage === 1 ? BASE_SOLIDS : ['Cilindro-Cone'] as SolidType[];
    ...
    <div className="min-h-screen bg-stone-950 text-stone-200 font-pixel flex flex-col overflow-hidden relative">
      ...
      <main className="content-layer flex-1 grid grid-cols-1 lg:grid-cols-4 gap-8 lg:gap-12 p-4 lg:p-8 relative">
        ...
        <div className="flex flex-col items-center justify-between relative lg:col-span-2">
          ...
          {/* Action Panel */}
          <div className="w-full md:w-max min-w-[340px] max-w-4xl bg-stone-900/95 border-4 border-stone-700 p-6 sm:p-8 shadow-2xl mt-8 z-10 mx-auto">
            <form onSubmit={handleAttack} className="flex flex-col items-center gap-6">
              
              {/* Character Selector */}
              <div className="w-full mb-2">
                <p className="text-center text-stone-300 mb-5 text-[11px] md:text-sm tracking-widest font-bold">QUEM RECEBERÁ O XP?</p>
                <div className="flex flex-row justify-center gap-4 sm:gap-6 w-full">
                  {characters.map(char => {
                    const isSelected = selectedCharId === char.id;
                    return (
                      <div key={char.id} className="flex flex-col items-center">
                        <p className={`text-[10px] md:text-xs font-bold uppercase mb-2 ${isSelected ? char.color : 'text-stone-500'}`}>
                          {char.role}
                        </p>
                        <button
                          type="button"
                          onClick={() => setSelectedCharId(char.id)}
                          className={`relative overflow-hidden border-2 transition-all duration-300 ${isSelected ? `${char.borderColor} scale-110 ${char.glowColor}` : 'border-stone-700 opacity-60 hover:opacity-100 hover:border-stone-400'}`}
                        >
                          <CharacterAvatar role={char.role} className="w-16 h-16 sm:w-20 sm:h-20" />
                          {isSelected && (
                            <div className={`absolute inset-0 ${char.bgColor}`} />
                          )}
                        </button>
                        <span className={`text-[11px] md:text-xs mt-3 font-pixel ${isSelected ? char.color : 'text-stone-500'}`}>
                          {char.xp} XP
                        </span>
                      </div>
                    );
            ...
            <div className="bg-stone-900/95 border-4 border-stone-600 p-8 w-full max-w-sm shadow-2xl flex flex-col items-center">
              ...
              <div className="w-full space-y-4 text-xs md:text-sm">
                {Object.entries(currentProblem.dimensions).map(([key, value]) => (
                  <div key={key} className="flex justify-between items-center bg-stone-950 p-4 border-2 border-stone-800 gap-4">
                    <span className="text-stone-400 break-words flex-1 text-left">{getDimensionLabel(key)}</span>
